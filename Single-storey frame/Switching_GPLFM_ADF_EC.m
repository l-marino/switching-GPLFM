...
% and (discontinuous) nonlinear force of a base-excited mass-spring-damper 
...
Xf = 1 + p;                             % Latent force vector size
...
pCov = zeros(X,X,S);
for s=1:S
    pCov(:,:,s) = [2e-3 0 0; 0 0.05 0; 0 0 0.2];        % To be defined by the user
end
        ...
        for itm = 1:I
            for st = 1:S
                ...
                if st ~= 2          % Excluding sticking model
                    ...
                    mu_pred_t  = [mu_upd(1,itm,stm,t-1); 0;...
                        k*u(1,t-1) + c*u(2,t-1) - k*mu_upd(1,itm,stm,t-1);];
                ...
                log_p_yt_g_st_stm_itm_y1tm = ...
                    - 0.5*(r'/G*r + trace(logm(G)) + Y*log(2*pi));
                ...
                for st=1:S
                    for it=1:I
                        ...
                        for jtp = 1:J
                            ...
                            Cov_xt_y1T(:,:,jtp,stp,it,st) = leftA*cov*leftA';
